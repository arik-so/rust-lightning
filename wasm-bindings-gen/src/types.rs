...
impl<'a, 'c: 'a> TypeResolver<'a, 'c> {
	...
	/// not require a generated container class.
	...
	pub fn resolve_path(&self, p: &syn::Path, generics: Option<&GenericTypes>) -> String {
		let path_message = format!("Failed to resolve path: {:?}", p);
		self.maybe_resolve_path(p, generics).expect(path_message.as_str())
	}
