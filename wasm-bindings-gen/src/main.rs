	...
	match &format!("{}", macro_path.segments.iter().next().unwrap().ident) as &str {
		"impl_writeable" | "impl_writeable_len_match" => {
			...
			if let Some(s) = types.maybe_resolve_ident(&struct_for) {
				...
				writeln!(w, "#[wasm_bindgen]").unwrap();
				writeln!(w, "pub fn {}_write(obj: *const {}) -> crate::c_types::derived::CVec_u8Z {{", struct_for, struct_for).unwrap();
	...
	for item in t.items.iter() {
		match item {
			&syn::TraitItem::Method(ref m) => {
				...
				write!(w, "\tpub {}: js_sys::Function", m.sig.ident).unwrap();
				generated_fields.push(format!("{}", m.sig.ident));
...
fn convert_file<'a, 'b>(libast: &'a FullLibraryAST, crate_types: &mut CrateTypes<'a>, in_dir: &str, out_dir: &str, path: &str, orig_crate: &str, module: &str, header_file: &mut File, cpp_header_file: &mut File) {
	...
	if path.ends_with("/lib.rs") {
		...
		// add wasm compilation dependencies
		writeln!(out, "extern crate js_sys;").unwrap();
		writeln!(out, "extern crate serde;").unwrap();
		writeln!(out, "extern crate web_sys;").unwrap();
		writeln!(out, "use serde::{Serialize, Deserialize};").unwrap();
		writeln!(out, "use wasm_bindgen::prelude::*;").unwrap();
		writeln!(out, "use wasm_bindgen::__rt::std::io::Cursor;").unwrap();
...
fn main() {
	...
	let mut derived_templates = std::fs::OpenOptions::new().write(true).create(true).truncate(true)
		.open(&args[4]).expect(format!("Unable to open new header file: {}", &args[4]).as_str());
	let mut header_file = std::fs::OpenOptions::new().write(true).create(true).truncate(true)
		.open(&args[5]).expect(format!("Unable to open new header file: {}", &args[5]).as_str());
	let mut cpp_header_file = std::fs::OpenOptions::new().write(true).create(true).truncate(true)
		.open(&args[6]).expect(format!("Unable to open new header file: {}", &args[6]).as_str());
