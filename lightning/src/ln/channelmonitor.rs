...
pub struct ChannelMonitorUpdate {
	...
	/// sequence number (and updates may panic if they are not).
	///
...
impl<ChanSigner: ChannelKeys> ChannelMonitor<ChanSigner> {
	...
	pub fn block_connected(&mut self, txn_matched: &[&Transaction], height: u32, block_hash: &Sha256dHash, broadcaster: &BroadcasterInterface, fee_estimator: &FeeEstimator)-> Vec<(Sha256dHash, Vec<TxOut>)> {
	...
	/// and an explicit reference to a transaction broadcaster and fee estimator.
	pub fn block_disconnected(&mut self, height: u32, block_hash: &Sha256dHash, broadcaster: &BroadcasterInterface, fee_estimator: &FeeEstimator) {
...
impl<R: ::std::io::Read, ChanSigner: ChannelKeys + Readable<R>> ReadableArgs<R, Arc<Logger>> for (Sha256dHash, ChannelMonitor<ChanSigner>) {
	fn read(reader: &mut R, logger: Arc<Logger>) -> Result<Self, DecodeError> {
		...
		let mut pending_events = Vec::with_capacity(cmp::min(pending_events_len as usize, MAX_ALLOC_SIZE / std::mem::size_of::<events::Event>()));
		...
		let mut outputs_to_watch = HashMap::with_capacity(cmp::min(outputs_to_watch_len as usize, MAX_ALLOC_SIZE / (32 + 3*8)));
