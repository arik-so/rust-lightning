...
use ln::peer_handler::{InitSyncTracker, MessageHandler, Peer, PeerHandleError, SocketDescriptor};
...
pub struct PeerCoordinator<Descriptor: SocketDescriptor> {
	pub(crate) message_handler: MessageHandler,
	pub(crate) peers: Mutex<PeerHolder<Descriptor>>,
	pub(crate) our_node_secret: SecretKey,
...
pub(crate) struct PeerHolder<Descriptor: SocketDescriptor> {
	pub(crate) peers: HashMap<Descriptor, Peer>,
	...
	pub(crate) node_id_to_descriptor: HashMap<PublicKey, Descriptor>,
}
...
impl<Descriptor: SocketDescriptor> PeerCoordinator<Descriptor> {
	/// Constructs a new PeerManager with the given message handlers and node_id secret key
	/// ephemeral_random_data is used to derive per-connection ephemeral keys and must be
	/// cryptographically secure random bytes.
	pub fn new(message_handler: MessageHandler, our_node_secret: SecretKey, ephemeral_random_data: &[u8; 32], logger: Arc<Logger>) -> PeerCoordinator<Descriptor> {
	...
	pub fn process_events(&self) {
		{
			...
			let mut peers_lock = self.peers.lock().unwrap();
			let peer_holder = &mut *peers_lock;
			for event in events_generated.drain(..) {
				macro_rules! get_peer_for_forwarding {
					($node_id: expr, $handle_no_such_peer: block) => {
						{
							let descriptor = match peer_holder.node_id_to_descriptor.get($node_id) {
							...
							match peer_holder.peers.get_mut(&descriptor) {
								Some(peer) => {
				...
				match event {
					...
					MessageSendEvent::BroadcastChannelAnnouncement { ref msg, ref update_msg } => {
						...
						if self.message_handler.route_handler.handle_channel_announcement(msg).is_ok() && self.message_handler.route_handler.handle_channel_update(update_msg).is_ok() {
							...
							for (ref descriptor, ref mut peer) in peer_holder.peers.iter_mut() {
					...
					MessageSendEvent::HandleError { ref node_id, ref action } => {
						match *action {
							msgs::ErrorAction::DisconnectPeer { ref msg } => {
								if let Some(mut descriptor) = peer_holder.node_id_to_descriptor.remove(node_id) {
									peer_holder.peers_needing_send.remove(&descriptor);
									if let Some(mut peer) = peer_holder.peers.remove(&descriptor) {
			...
			for mut descriptor in peer_holder.peers_needing_send.drain() {
				match peer_holder.peers.get_mut(&descriptor) {
